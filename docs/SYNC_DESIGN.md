...
`calendar_id` 记录上次同步选中的日历：定位日历时**优先按此 id**（仅当其标题仍等于
配置的 calendar_name 时生效），找不到再按名字、最后新建。这保证同名日历并存或
系统返回顺序变化时，不会在两个日历间来回切换（否则上次建的事件会被误判为
"用户删除"并批量回读成请假）。

## 3. 同步流程（单次 Sync 的五步）

0. **同步窗口**：最早锚点日（最早规则生效日 / 历史 / 导入日中的最小者）→ 当月月底
   （再延伸到最晚 override 日期）——整段排班史都物化在日历中，而非只有未来；
   `next_month` 模式则只取下个整月（月末预生成用）。
1. **计算期望态**：ShiftlyKit 由 rules + swaps + leave + holidays 求出窗口内的计划班次集合 `P`。
2. **读取日历态**：取 Shifts 日历窗口内全部事件 `E`（按年分块谓词查询并按 id 去重，
   规避 EventKit 约 4 年的谓词上限）。
...
- **事件被删除** → `leave.json` 追加单日请假（UI 上区分显示为"日历侧移除"）。
  例外（状态失效保护）：state 跟踪的事件 ≥2 且**全部**消失时，视为日历被删除/
  重建/切换而非逐日删班——丢弃旧 state 直接重建（认领/新建），不写任何请假。
